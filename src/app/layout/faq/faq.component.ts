...
verifiactionForm:FormGroup
  ...
  constructor(public router: Router, private fb: FormBuilder,private adminService:AdminService,public dialog: MatDialog,vcr: ViewContainerRef,
    ...
    this.verifiactionForm = fb.group({
                'question': [null, Validators.compose([Validators.required,Validators.maxLength(1000)])],
                'answer': [null, Validators.compose([Validators.required,Validators.maxLength(1000)])],
...
onSubmit(){
  if (this.datamodel.id) {
      this.adminService.EditFaq(this.datamodel)
        .subscribe(data=>{
            if(data.response){;
      ...
      this.adminService.AddFaq(this.datamodel)
        .subscribe(data=>{
            if(data.response){;
             this.onGetList()
    ...
    this.adminService.DeleteFaq(id)
        .subscribe(data=>{
            if(data.response){;
              ...
              this.toastr.success('Faq deleted' ,'Success',{toastLife: 2000, showCloseButton: true});
              this.users=[]
             if (this.AboutUSlist.length>5){
                  this.pageSize=5
                }else{
